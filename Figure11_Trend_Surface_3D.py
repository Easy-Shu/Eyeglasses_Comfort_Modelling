...
p=ax.plot_surface(X,Y, z[1:,:], rstride=1, cstride=1, cmap='PiYG',
                  alpha=1,edgecolor='k',linewidth=0.25)

ax.set_xlabel("Nosepad width scale factor (2 mm)")
ax.set_ylabel("Temple clamping force (N)")
ax.set_zlabel('Static Perceived Overall Comfort',)
ax.set_zlim(-3,3)


cbar=fig.colorbar(p, shrink=0.5,aspect=10)
cbar.set_label('Static Perceived Overall Comfort',)
...
ax.set_xlabel("Nosepad width scale factor (2 mm)")
ax.set_ylabel("Temple clamping force (N)")
ax.set_zlabel('Dynamic Perceived Overall Comfort',)
ax.set_zlim(-3,3)


cbar=fig.colorbar(p, shrink=0.5,aspect=10)
cbar.set_label('Dynamic Perceived Overall Comfort',)
...
ax.set_xlabel("Nosepad width scale factor (2 mm)")
ax.set_ylabel("Temple clamping force (N)")
ax.set_zlabel('Global Perceived Overall Comfort',)
ax.set_zlim(-3,3)


cbar=fig.colorbar(p, shrink=0.5,aspect=10)
cbar.set_label('Perceived Overall Comfort',)


#==============================================testing==============================
df_test=df_data0[df_data0["Test_order"]==7]
df_test["test_id"]=range(df_test["ID"].shape[0])
df_subNosepad=df_test[["Nosepad_Interval","Force_Nosepad","OverallNosepad_Comfort_S"]].values
df_subTemple=df_test[["Nosepad\nInterval","Clapping_Force","OverallFrame_Comfort_S"]].values

df_testS=pd.DataFrame(data=np.r_[df_subNosepad,df_subTemple],
                                  columns=["Nosepad","Clapping_Force","Comfort"])

df_testS["Temple"]=np.round(df_testS["Clapping_Force"]/force_interval,0)*force_interval

df_testS["predict_comfort"]=model_S.predict(df_testS)
df_testS["d"]=np.abs(df_testS["predict_comfort"]-df_testS["Comfort"])
print(df_testS["d"].mean(),df_testS["d"].std())


df_subNosepad=df_test[["Nosepad_Interval","Force_Nosepad","OverallNosepad_Comfort_D"]].values
df_subTemple=df_test[["Nosepad\nInterval","Clapping_Force","OverallFrame_Comfort_D"]].values

df_testD=pd.DataFrame(data=np.r_[df_subNosepad,df_subTemple],
                                  columns=["Nosepad","Clapping_Force","Comfort"])

df_testD["Temple"]=np.round(df_testD["Clapping_Force"]/force_interval,0)*force_interval

df_testD["predict_comfort"]=model_D.predict(df_testD)
df_testD["d"]=np.abs(df_testD["predict_comfort"]-df_testD["Comfort"])
print(df_testD["d"].mean(),df_testD["d"].std())


df_testG=df_testS.append(df_testD)
df_testG["predict_comfort"]=model_G.predict(df_testG)
df_testG["d"]=np.abs(df_testG["predict_comfort"]-df_testG["Comfort"])
print(df_testG["d"].mean(),df_testG["d"].std())
